...
const stats = [
  ...
  ['pendingTeachers','Pending requests',Users],['approvedTeachers','Approved teachers',Users],
  ['rejectedTeachers','Rejected teachers',Users],
  ['recentApprovals','Recent approvals',Users],['recentRegistrations','Recent registrations',GraduationCap],
]
...
export default function AdminDashboard() {
  ...
  return <section className="admin-page">
    ...
    {error ? <div className="admin-error">{error}</div> : !data ? <div className="admin-empty">Loading platform telemetry...</div> : <>
      <div className="admin-grid">{stats.map(([key,label,Icon])=>{const value=Array.isArray(data[key])?data[key].length:data[key];return <article className="admin-stat" key={key}><div className="admin-stat-top"><span>{label}</span><Icon size={17}/></div><strong>{Number(value||0).toLocaleString()}</strong><small>Live platform total</small></article>})}</div>
